...
var Slider = function (_a) {
    ...
    var percent = Math.ceil(((parseInt(value) - (parseInt(min) || 0)) /
        ...
        100);
    return (_jsxs("div", __assign({ className: "input relative w-full " + className }, { children: [_jsx("input", { className: "slider-number block w-8 border-2 outline-none m-auto mb-4 text-center font-bold " + (disabled ? 'text-dark-gray-200 cursor-not-allowed' : 'text-blue') + " " + (onFocus ? 'focused' : '') + " " + (dark ? 'bg-light-gray' : 'bg-white'), type: "number", value: value, min: min || '0', max: max || '100', onChange: onChange, onFocus: function () { return setOnFocus(true); }, onBlur: function () { return setOnFocus(false); } }, void 0),
            _jsx("input", __assign({ style: disabled
                    ? {
                    ...
                    }, className: "slider appearance-none w-full h-4  rounded-full outline-none " + (disabled ? 'cursor-not-allowed' : '') + " " + (onFocus ? 'focused' : ''), type: "range", disabled: disabled, min: min || 0, max: max || 100, value: value, onChange: onChange, onFocus: function () { return setOnFocus(true); }, onBlur: function () { return setOnFocus(false); }, id: sliderId }, otherProps), void 0),
            _jsxs("div", __assign({ className: "values flex w-full justify-between text-dark-gray text-xs font-bold" }, { children: [_jsx("span", { children: min || '0' }, void 0),
                    _jsx("span", { children: max || '100' }, void 0)] }), void 0)] }), void 0));
};
